...
- (id)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        if (!IH_IS_IPHONE) {
            self.legendView = [[LegendView alloc] initWithFrame:CGRectMake(frame.size.width - 50 - 10, 10, 80, 100)];
        }
...
//        self.xAxisLabel.alpha = 0.0;
        ...
        if (!IH_IS_IPHONE) {
            self.scaleFont = [UIFont systemFontOfSize:14.0];
            self.xAxisLabel.font = [UIFont boldSystemFontOfSize:14];
...
- (void)setData:(NSArray *)data {
    if(data != _data) {
        ...
        if (data.count == 1 && [titles[0] isEqualToString:@""]) {
